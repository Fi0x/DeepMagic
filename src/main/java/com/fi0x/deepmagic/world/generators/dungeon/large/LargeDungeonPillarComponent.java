...
public class LargeDungeonPillarComponent extends LargeDungeonBaseComponent
{
    //TODO: Adapt code for dungeon
    protected PillarType type;

    // need default constructor for loading by minecraft
    public LargeDungeonPillarComponent()
    {
        super();
    }

    public LargeDungeonPillarComponent(TemplateManager templateManager, Template template, String templateName, boolean fillBelow, BlockPos position, Rotation rot, Mirror mi, PillarType pillar)
    {
        super(templateManager, template, templateName, fillBelow, position, rot, mi);
        type = pillar;
    }

    public LargeDungeonPillarComponent(TemplateManager templateManager, String templateName, boolean fillBelow, BlockPos position, Rotation rot, Mirror mi, PillarType pillar)
    {
        super(templateManager, templateManager.get(FMLCommonHandler.instance().getMinecraftServerInstance(), new ResourceLocation(Reference.MOD_ID, templateName)), templateName, fillBelow, position, rot, mi);
        type = pillar;
    }
    ...
    protected boolean isReplaceable(World world, BlockPos pos)
    {
        ...
        else
        {
            ...
            else if(state.getBlock() == TABlocks.STONE)
            {
                ...
                return type == StoneType.STONE_VOID || type == StoneType.STONE_TAINT_NODECAY || type == StoneType.SOIL_STONE_TAINT_NODECAY;
            } else
                return false;
        }
    }
                            ...
                            if(boundingBox.isVecInside(mutable) && !world.isAirBlock(mutable))
                            {
                                ...
                                if(diff >= 0)
                                {
                                    ...
                                    for(int y = boundingBox.maxY + 1; y < 256; ++y)
                                    {
                                        ...
                                        setWardedBlockState(world, mutable, up);
                                    }
                                }
                            }
                        }
